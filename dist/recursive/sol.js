...
import { getLengthNoWhitespace, getLineCounter, willFillChunkSize, } from "../utils.js";
import { splitOnSeparator } from "./lib.js";
export let splitSol = (text, params) => {
    ...
    for (let i = 0; i < preSplit.length; i++) {
        ...
        let docs = splitOnSeparator(chunk, separators[0], separators, chunkBuilder, lineCounter, params);
...
export let preSplitSol = (text, chunkSize, chunkOverlap) => {
...
export const splitOnSolComments = (text) => {
    let builder = [];
    let split = text.split("\n");
    const isBlockCommentStart = (line) => line.match(/^(\s+|)\/\*/);
    let isSlashComment = (line) => line.match(/^(\s+|)\/{3}/);
